...
//    frequencyRate_ = frequencyRate;
  ...
  bool calculatePlan(geometry_msgs::PoseStamped& targetPose){
    std::cout << std::endl;
    ROS_DEBUG_STREAM("BEGIN calculatePlan");
...
//    }
    ...
    ROS_DEBUG_STREAM("**************** BEGINNING CALCULATING PLAN ****************");
...
//    ROS_DEBUG_STREAM("BASE->PROJECTION TF MSG" << transformString(base2ProjectionTFMsg));
//    ROS_DEBUG_STREAM("BASE->PROJECTION TRANSFORM" << transformString(base2ProjectionTF));
//    ROS_DEBUG_STREAM("BASE->PROJECTION POSE MSG" << poseString(base2ProjectionPose.pose));
...
//    target2EntrancePose.header.frame_id =
    ...
    ROS_DEBUG_STREAM("BASE->ENTRANCE TRANSFORM" << transformString(base2EntranceTFMsg));


    ROS_DEBUG_STREAM("ORIGINAL TARGET POSE MSG" << poseString(targetPose));
    ...
    ROS_DEBUG_STREAM("TARGET POSE MSG + ENTRANCE DISTANCE" << poseString(targetPose));


    // Transform targetPose into base_link frame
    try {
      ROS_DEBUG_STREAM("TRANSFORMING TARGET POSE FROM " << targetPose.header.frame_id << " TO " << robotFrameID);
      ...
      ROS_DEBUG_STREAM("TRANSFORMED BASE->TARGET TF MSG " << transformString(base2TargetTFMsg));
//      ROS_DEBUG_STREAM("TRANSFORMED BASE->TARGET TRANSFORM " << transformString(base2TargetTF));
      ROS_DEBUG_STREAM("TRANSFORMED BASE->TARGET POSE MSG " << poseString(base2TargetPose.pose));

//      ROS_DEBUG_STREAM("TRANSFORM FOUND BETWEEN base_link and " << targetPose.header.frame_id);
//      robotTFMsg = tfBuffer_.lookupTransform("odom", "base_link", ros::Time(0));
    ...
    ROS_DEBUG_STREAM("GETTING RADIAL DISTANCE TO GOAL: " << currentGoalDist);
    ...
    while ((projGoalDist > goal_dist_tolerance_) || (fabs(deltaAngle) > goal_orientation_tolerance_))
    {
      ...
      ROS_DEBUG_STREAM("PROJ RADIAL DIST TO GOAL = " << projGoalDist << " > " << goal_dist_tolerance_);
...
//      deltaAngle = getDeltaAngle(proj2TargetTF);
      ROS_DEBUG_STREAM("GETTING DELTA ANGLE " << deltaAngle);
...
//        ROS_DEBUG_STREAM("GOAL LESS THAN 0.5 SETTING MAX V TO " << v);
//      }
//      else
//      {
//        v = std::min(lin_vel_max_, std::max(lin_vel_min_, v));
//        ROS_DEBUG_STREAM("GOAL GREATER THAN 0.5 SETTING MIN V TO " << v);
//      }
//      // Compute angular velocity
//      double omega = curvature * v;
//      ROS_DEBUG_STREAM("CALCULATING OMEGA BASED ON CURVATURE " << omega);
//      // Bound angular velocity
      double omegaBounded = std::min(omega_max_, std::max(-omega_max_, omega));
      ROS_DEBUG_STREAM("BOUNDING OMEGA TO  " << omegaBounded);
//      // Make sure that if we reduce w, we reduce v so that curvature is still followed
//      if (omega != 0.0) {
//        ROS_DEBUG_STREAM("OMEGA NOT ZERO ");
//        ROS_DEBUG_STREAM("V = " << v << " * (" << omegaBounded << " / " << omega << ") = " << v * (omegaBounded/omega));
...
//      twist.linear.x = v;
      ...
      ROS_DEBUG_STREAM("TWIST: " << twistString(twist));
//      if(projGoalDist < entrance_dist_){
//        twist.linear.x *= 0.25;
//        twist.angular.z *= 0.25;
//        ROS_DEBUG_STREAM("Approaching Target - Twist Scaled Down To " << twistString(twist));
//      }
      if(steps == 0){
        currentTwist = twist;
      }
...
//      ROS_DEBUG_STREAM("ADDING POSE TO PLAN" << poseString(base2ProjectionPose.pose));
//      addtoPlan(base2ProjectionPose,twist);

//      ROS_DEBUG_STREAM("STEPPING POSE");
...
//      stepPose(base2ProjectionPose,twist);
      ROS_DEBUG_STREAM("STEPPED  " << poseString(base2ProjectionPose.pose));



      ROS_DEBUG_STREAM("GETTING RADIAL DISTANCE BETWEEN UPDATED TARGET & STEPPED POSE");
      ...
      ROS_DEBUG_STREAM("RADIAL GOAL DISTANCE " << gdSS.str());

      ROS_DEBUG_STREAM("END PATH STEP #" << steps);
...
//      std::cout << std::endl;
  ...
  ROS_DEBUG_STREAM("DOCK POSE CALLBACK");

    geometry_msgs::PoseStamped pose = *msg;
//    if(calculateCurrentApproach(pose))
    if(calculatePlan(pose))
    {
      ROS_DEBUG_STREAM("SUCCESSFULLY GENERATED PATH");
    }
    else {
...
// ******************************** DELTA ********************************
  ...
  double getDeltaAngle(tf2::Transform p2tTF){
    ...
    ROS_DEBUG_STREAM("Delta Angle: Robot/Projection X Vector " << vectorString(vecProjX));
    // LOS Vector in base_link / projected base_link frame
    tf2::Vector3 vecLOS = p2tTF.getOrigin();
    ROS_DEBUG_STREAM("Delta Angle: Line Of Sight Vector " << vectorString(vecLOS));
    da = vecLOS.angle(vecProjX);
    ROS_DEBUG_STREAM("Delta Angle: vecLOS.angle(vecRobotX) " << da);

    da = tf2::tf2Angle(vecLOS,vecProjX);
    ROS_DEBUG_STREAM("Delta Angle: tf2::tf2Angle(vecLOS,vecProjX) " << da);

    da = tf2::getYaw(p2tTF.getRotation());
    ROS_DEBUG_STREAM("DELTA Angle: tf2::getYaw(p2tTF.getRotation()) " << da);

//    tfLOS.setOrigin(vecLOS);
//    tfTX.setOrigin(vecProjX);

//    da = tfLOS.getRotation().angleShortestPath(tfTX.getRotation());
//    ROS_DEBUG_STREAM("Delta Angle: tfLOS.getRotation().angleShortestPath(tfTX.getRotation()) " << da);

    return da;
  }
  double getDeltaControl(double& phi){
    ROS_DEBUG_STREAM("GETTING DELTA CONTROL = atan(- " << kPhi_ << " * " << phi << ") = atan(- " << kPhi_ * phi << ") " << " = " << (std::atan(-kPhi_ * phi)));
...
// ******************************** PHI ********************************
  ...
  double getPhi(tf2::Transform p2tTF){
    ...
    // X Axis Vector in base_link / projected base_link frame
    tf2::Vector3 vecProjX(1,0,0);
//    ROS_DEBUG_STREAM("Phi Angle: Robot/Projection X Vector " << vectorString(vecProjX));
    tf2::Vector3 vecProj2Target = p2tTF.getOrigin();
//    ROS_DEBUG_STREAM("Phi Angle: Robot/Projection --> Target Vector " << vectorString(vecProj2Target));
    tf2::Vector3 vecTargetX = vecProjX * vecProj2Target;
//    ROS_DEBUG_STREAM("Phi Angle: Target X Vector =  vecProjX * vecProj2Target" << vectorString(vecTargetX));

    tf2::Vector3 vecTargetXRotated = tf2::quatRotate(p2tTF.getRotation(),vecTargetX);
//    ROS_DEBUG_STREAM("Phi Angle: Target X Vector Rotated by Q " << vectorString(vecTargetXRotated));

    phi = vecProj2Target.angle(vecTargetXRotated);
    ROS_DEBUG_STREAM("Phi Angle: vecProj2Target.angle(vecTargetX) " << phi);

    tfPT.setOrigin(vecProj2Target);
    tfTX.setOrigin(vecTargetXRotated);

//    phi = tfPT.getRotation().angleShortestPath(tfTX.getRotation());
//    ROS_DEBUG_STREAM("Phi Angle: tfPT.getRotation().angleShortestPath(tfTX.getRotation()) " << phi);

//    phi = tfTX.getRotation().angleShortestPath(tfPT.getRotation());
//    ROS_DEBUG_STREAM("Phi Angle: tfTX.getRotation().angleShortestPath(tfPT.getRotation()) " << phi);
...
//    ROS_DEBUG_STREAM("Phi Angle: tf2::getYaw(p2tTF.getRotation()) " << phi);
//    return phi;
//  }
...
//    ROS_DEBUG_STREAM("Getting Curvature");
    double k, z, left, right, rightDenom;
    z = getZ(deltaAngle, deltaControl);

    left = kDelta_ * z;
//    ROS_DEBUG_STREAM("GETTING LEFT = " << kDelta_ << " * " << z << " = " << left);

    rightDenom = 1 + ( (kPhi_*phi)*(kPhi_*phi) );
//    ROS_DEBUG_STREAM("GETTING RIGHT DENOM = 1 + (" << (kPhi_*phi) << " * " << (kPhi_*phi) << ") = " << rightDenom);

    right = kPhi_ / rightDenom;
//    ROS_DEBUG_STREAM("GETTING RIGHT = " << kPhi_ << " / " << rightDenom << " = " << right);
//    ROS_DEBUG_STREAM("GETTING RIGHT = " << 1 << " + " << right << " = " << (1 + right));
    right = 1 + right;
//    ROS_DEBUG_STREAM("GETTING RIGHT = " << right << " * sin(" << deltaAngle << ") = " << right << " * " << sin(deltaAngle) << " = " << (right * sin(deltaAngle)));
    right = right * sin(deltaAngle);


    k = left + right;
//    ROS_DEBUG_STREAM("GETTING CURVATURE = " << left << " + " << right << " = " << k);
//    ROS_DEBUG_STREAM("GETTING CURVATURE = " << k << " * (-1) = " << ((-1) * k) );
    k = (-1) * k;
//    ROS_DEBUG_STREAM("GETTING CURVATURE = " << k << " / " << goalDist << " = " << (k / goalDist));
    k = k / goalDist;
...
//    ROS_DEBUG_STREAM("Calculating OMEGA with Constant Linear Velocity ->" << lin_vel_min_);
...
//    ROS_DEBUG_STREAM("GETTING RIGHT = " << right << " * sin(" << deltaAngle << ") = " << right << " * " << sin(deltaAngle) << " = " << (right * sin(deltaAngle)));
    ...
    omega = left + right;
//    ROS_DEBUG_STREAM("GETTING OMEGA = " << left << " + " << right << " = " << omega);
//    ROS_DEBUG_STREAM("GETTING OMEGA = " << omega << " * (-1) = " << ((-1) * omega) );
    omega = (-1) * omega;
//    ROS_DEBUG_STREAM("GETTING OMEGA = " << omega << " / " << goalDist << " = " << (omega / goalDist));
    omega = omega / goalDist;
    ROS_DEBUG_STREAM("GETTING OMEGA = " << omega << " * " << lin_vel_min_ << " = " << (omega *lin_vel_min_));
...
//    ROS_DEBUG_STREAM("GETTING Z = " << deltaAngle << " - " << deltaControl << " = " << z);
    return z;
  }
  ...
  void stepPose(geometry_msgs::PoseStamped& poseOld, geometry_msgs::Twist& t){
    ROS_DEBUG_STREAM("Started Stepping Pose with Twist " << twistString(t));
    ...
    ROS_DEBUG_STREAM("ORIG  Pose           " << poseString(poseOld.pose) << " YAW: " << yawOld);

    ROS_DEBUG_STREAM("DELTA Pose (YAW calc)" << poseString(poseDelta.pose)  << " YAW: " << yawDelta);
    tf2::convert(qDelta, poseDelta.pose.orientation);
    ROS_DEBUG_STREAM("DELTA Pose (Qua calc)" << poseString(poseDelta.pose)  << " YAW: " << yawDelta);

    ROS_DEBUG_STREAM("STEPD Pose (YAW calc)" << poseString(poseNew.pose)  << " YAW: " << yawNew);
    tf2::convert(qNew, poseNew.pose.orientation);
    ROS_DEBUG_STREAM("STEPD Pose (Qua calc)" << poseString(poseNew.pose)  << " YAW: " << yawNew);

    poseOld.pose = poseNew.pose;
    ROS_DEBUG_STREAM("Finished Stepping Pose");
  }
...
//    deltaY = (t.linear.x * sin(deltaYaw)  +  t.linear.y * cos(deltaYaw)) * time_step_;
    ...
    ROS_DEBUG_STREAM(positionSS.str());

    ROS_DEBUG_STREAM("ORIG  TF " << transformString(curTF) <<  yawString(yawOld));
    ROS_DEBUG_STREAM("DELTA TF " << transformString(deltaTF)  << yawString(deltaYaw));

    return deltaTF;
//    tf2::convert(qDelta, poseDelta.pose.orientation);
//    ROS_DEBUG_STREAM("DELTA TF (Qua calc)" << poseString(poseDelta.pose)  << " YAW: " << yawDelta);
  }
  ...
  tf2::Transform getProjectionToTargetTF(tf2::Transform& base2Proj, tf2::Transform& base2Target){
    ...
    ROS_DEBUG_STREAM("GETTING TF FROM PROJECTION TO TARGET " << transformString(proj2Target));
    return proj2Target;
  }
